...
class DMModelHelper {
  ...
  DMModelHelper(this._mongoConfig);
    ...
    Future<void> createIndex() async=>await DmModelController(mongoConfig: _mongoConfig).createIndex();


  Future<String> createDmRoom(DmModel dmModel) async {
    return await DmModelController(mongoConfig: _mongoConfig).addData(dmModel,docId: dmModel.id);
  }

  Future<List<DmModel>> getDmRooms(List<String> ids) async {
    return await DmModelController(mongoConfig: _mongoConfig).getDataFromIds(ids);
  }

  Stream<List<DmModel>> getDmRoomsStream(List<String> ids) {
    return DmModelController(mongoConfig: _mongoConfig).getDataStreamFromIds(ids);
  }
  ...
  Future<void> addMessage(Message message, String dmRoomId) async {
    ...
    await DmModelController(mongoConfig: _mongoConfig).updateData(update, dmRoomId);
  }
}
